...
- **Schema Migrations**: Database structure updates managed through Admin API endpoints
- **Docker Containers**: Portable, isolated deployment that runs consistently across development, staging, and production

### What INVI is NOT

INVI focuses exclusively on inventory management. It is **not**:
- ❌ Order Management System (OMS)
- ❌ Fulfillment Management System (FMS)  
- ❌ Warehouse Management System (WMS)

INVI **can** integrate with these systems via its REST API, serving as the inventory ledger they depend on.
...
- `POST /api/assignments/detailed` - Query inventory with location breakdown
  ...
  - Includes location paths, sort keys, and assignment IDs
...
- `POST /api/assignments/cycle-count` - Reconcile physical counts
  ...
  - Auto-resolves shortages (optional): reduces low-priority assignments first
  - Requires manual resolution for surpluses or when auto-resolution disabled
...
Found a bug? Please [open an issue](https://github.com/mak-m/public-repo/issues/new) with:
...
Have an idea? [Create a feature request](https://github.com/mak-m/public-repo/issues/new) describing:
...
Questions, suggestions, or comments? Open an issue on [GitHub Issues](https://github.com/mak-m/public-repo/issues).
